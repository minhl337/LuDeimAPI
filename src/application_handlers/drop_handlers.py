...
def drop_item(params, _id, conn, logger, config, session):
	try:
		...
		with conn:
			...
			if params[
				"item_uuid"] not in user.item_uuids | user.outgoing_item_uuids:
				return rpc.make_error_resp(0,
...
def drop_location(params, _id, conn, logger, config, session):
	try:
		...
		with conn:
			...
			if len(
					location.item_uuids | location.incoming_item_uuids | location.outgoing_item_uuids) != 0:
				return rpc.make_error_resp(0,
...
def drop_user(params, _id, conn, logger, config, session):
	try:
		...
		with conn:
			...
			if len(
					user.item_uuids | user.incoming_item_uuids | user.outgoing_item_uuids) != 0:
				return rpc.make_error_resp(0,
